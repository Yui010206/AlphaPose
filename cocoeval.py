...
class COCOeval:
    ...
    def computeOks(self, imgId, catId):
        ...
        for part in parts:
            ...
            if part == 'body':
                sigmas = sigmas[0 : body_foot_kpnum]
            elif part == 'face':
                sigmas = sigmas[body_foot_kpnum : body_foot_kpnum + face_kpnum]
            elif part == 'hand':
                sigmas = sigmas[body_foot_kpnum + face_kpnum:]
            ...
            for j, gt in enumerate(gts):
                ...
                if part == 'body':
                    g = np.array(gt['keypoints'][0 : body_foot_kpnum*3],dtype=np.float32)
                    #assert g.shape[0] == 23*3, g.shape
                elif part == 'face':
                    g = np.array(gt['keypoints'][body_foot_kpnum*3 : (body_foot_kpnum + face_kpnum)*3],dtype=np.float32)
                    #assert g.shape[0] == 68*3, g.shape
                elif part == 'hand':
                    g = np.array(gt['keypoints'][(body_foot_kpnum + face_kpnum)*3:],dtype=np.float32)
                ...
                for i, dt in enumerate(dts):
                    if part == 'body':
                        d = np.array(dt['keypoints'][0 : body_foot_kpnum*3],dtype=np.float32)
                        #assert d.shape[0] == 23*3, d.shape
                    elif part == 'face':
                        d = np.array(dt['keypoints'][body_foot_kpnum*3 : (body_foot_kpnum + face_kpnum)*3],dtype=np.float32)
                        #assert d.shape[0] == 68*3, d.shape
                    elif part == 'hand':
                        d = np.array(dt['keypoints'][(body_foot_kpnum + face_kpnum)*3:],dtype=np.float32)
